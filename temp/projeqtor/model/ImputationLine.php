...
// MTY - LEAVE SYSTEM
  ...
  static function drawLines($resourceId, $rangeType, $rangeValue, $showIdle, $showPlanned=true, $print=false, $hideDone=false, $hideNotHandled=false, $displayOnlyCurrentWeekMeetings=false, $currentWeek=0, $currentYear=0, $showId=false) {
    ...
    $totalCapacity=0;
    $allDate=array();
    $convertMaxDailyWork=Work::displayImputation($resource->maxDailyWork);
    $convertMaxWeeklyWork=Work::displayImputation($resource->maxWeeklyWork);
...
//       }
      ...
      if ( (! $isUnderConstruction and ! $isTemplate) or $line->realWork>0) {
        ...
        for ($i=1; $i<=$nbDays; $i++) {
          ...
          if($line->refType!='Project'){
            ...
            if($line->refType=='Ticket'){
              $obj = new Ticket($line->refId, true);
              if($obj->idle)$readonly=true;
            }
          }
          $convertCapacity=Work::getConvertedCapacity($resource->getCapacityPeriod($curDate));
        ...
        if ($line->imputable   or $line->isPlanningActivity) {
          if (!$print) {
            ...
            echo ' constraints="{min:0}"';
            echo '  style="width: 60px; text-align: center;'.(($line->idle or ($line->locked and ! $line->isPlanningActivity))?'color:#A0A0A0; xbackground: #EEEEEE;':'').' " ';
    ...
    if ($convertMaxWeeklyWork and round($totalWork, 2)>round($convertMaxWeeklyWork,2)) {
