  ...
  // send Ajax request
    ...
    dojo.xhrPost({
        ...
        load : function(data, args) {     
          ...
          if (   (pageShort=='objectDetail.php' && (destination=='detailDiv' || destination=='detailFormDiv')) 
...
function globalSave() {
  ...
  } else if (dijit.byId('dialogWorkTokenMarkup')
      && dijit.byId('dialogWorkTokenMarkup').open) {
    var button = dijit.byId('dialogworkTokenMarkupSubmit');
  } else if (dijit.byId('dialogWorkTokenClientContract')
      && dijit.byId('dialogWorkTokenClientContract').open) {
...
//var lstidTDDays= dojo.byId('lstRefresDay_'+currentDay).value;
  ...
  showWait();
  var url='../tool/saveAbsence.php?isCurrent=true&lstDays='+lstDays+'&lstDaysFlatList='+lstDaysFlatList+'&userId='+userId+'&workVal='+workVal+'&idProject='+idProject+'&actId='+actId+'&assId='+assId;
  dojo.xhrGet({
    url : url+'&csrfToken='+csrfToken,
    handleAs : "text",
    load : function(data){
...
function setBetweenTwoDateAbsences(userId){
  ...
  showWait();
  var url='../tool/saveAbsence.php?isBetweenToDate=true&startDate='+startDate+'&endDate='+endDate+'&userId='+userId+'&workVal='+workVal+'&idProject='+idProject+'&actId='+actId+'&assId='+assId;
  dojo.xhrGet({
    url : url+'&csrfToken='+csrfToken,
    handleAs : "text",
    load : function(data){
...
// PBER : removed this code as it was refreshing Button Div when entering new Screen, leading to some lazy response
...
function addVote(objClass,idObj,editorType,mode,idRule) {
  pauseBodyFocus();
  var callBack=function() {
    if (editorType=="CK" || editorType=="CKInline") { // CKeditor type
      ...
      dijit.byId("voteNoteEditor").set("height", (screen.height*0.4)+'px'); // Works on first time
      dojo.byId("voteNoteEditor_iframe").style.height=(screen.height*0.4)+'px'; // Works after first time
    }
    dijit.byId("dialogAddVote").show();
  };
  var params="&idObj=" + idObj + "&class=" + objClass + "&mode=" + mode + "&idRule=" + idRule;
  loadDialog('dialogAddVote', callBack, false, params);
}

function saveAddVote(){
...
function saveAttributionVote() {
  ...
  if (formVar.validate()) {
    loadContent("../tool/saveVotingAttributionVote.php",
        "resultDivMain", "attributionVoteForm", true, 'AttributionVote', false);
...
function removeAttributionVote(id) {
  ...
  actionOK=function() {
    loadContent("../tool/saveVotingAttributionVote.php?mode=delete&idAttributionVote=" + id,
        "resultDivMain", null, true, 'AttributionVote', false);
  };
...
function saveResourceSkill() {
  ...
  if (formVar.validate()) {
    loadContent("../tool/saveResourceSkill.php",
        "resultDivMain", "resourceSkillForm", true, 'ResourceSkill', false);
...
function removeResourceSkill(id) {
  ...
  actionOK=function() {
    loadContent("../tool/saveResourceSkill.php?mode=delete&idResourceSkill=" + id,
        "resultDivMain", null, true, 'ResourceSkill', false);
  };
...
function saveActivitySkill() {
  ...
  if (formVar.validate()) {
    loadContent("../tool/saveActivitySkill.php",
        "resultDivMain", "activitySkillForm", true, 'ActivitySkill', false);
...
function removeActivitySkill(id) {
  ...
  actionOK=function() {
    loadContent("../tool/saveActivitySkill.php?mode=delete&idActivitySkill=" + id,
        "resultDivMain", null, true, 'ActivitySkill', false);
  };
...
function setValueCHeckBoxForUser(){
  ...
  var tab= {"copyToWithActivityPrice":"isCheckedActivityPrice",
      ...
      suList.forEach(function(item){
        ...
        if(!hiddenRow.includes(item)){
          ...
          if(subEl['id_'+item] && dojo.getAttr("group_"+item, "class").includes('ganttExpandClosed')){
            subEl['id_'+item].forEach(function(el){hiddenRow.push(el);});
          }
        }else{
          if(subEl['id_'+item] && dojo.getAttr("group_"+item, "class").includes('ganttExpandClosed')){
            subEl['id_'+item].forEach(function(el){hiddenRow.push(el);});
