...
// END - ADD BY TABARY - NOTIFICATION SYSTEM      
  ...
  public function copyTo($newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments, $withLinks, $withAssignments = false, $withAffectations = false, $toProject = null, $toActivity = null, $copyToWithResult = false, $copyToWithActivityPrice=false, $copyToWithStatus = false) {
    global $debugTraceUpdates,$debugTraceHistory;
    scriptLog("for ".debugDisplayObj($this)."->copyTo(newClass=$newClass, newType=$newType, newName=$newName, newProject=$newProject, setOrigin=$setOrigin, withNotes=$withNotes, withAttachments=$withAttachments, withLinks=$withLinks, withAssignments=$withAssignments, withAffectations=$withAffectations, toProject=$toProject, toActivity=$toActivity, copyToWithResult=$copyToWithResult, copyToWithStatus=$copyToWithStatus)");
    if (isset($debugTraceUpdates) and $debugTraceUpdates==true) {
      if ( ! property_exists($this,'_noHistory') or (isset($debugTraceHistory) and $debugTraceHistory==true) )
        debugTraceLog("Start COPYTO for ".get_class($this)." #".$this->id);$startMicroTime=microtime(true);
    }
    self::setCopyInProgress();
    $result=$this->copySqlElementTo ( $newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments, $withLinks, $withAssignments, $withAffectations, $toProject, $toActivity, $copyToWithResult, $copyToWithStatus);
    if (isset($debugTraceUpdates) and $debugTraceUpdates==true) {
      if ( ! property_exists($this,'_noHistory') or (isset($debugTraceHistory) and $debugTraceHistory==true) )
  ...
  private function copySqlElementTo($newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments, $withLinks, $withAssignments = false, $withAffectations = false, $toProject = null, $toActivity = null, $copyToWithResult = false, $copyToWithStatus = true) {
    $newObj = new $newClass ();
    $newObj->id = null;
    ...
    if (getLastOperationStatus($result)=='OK' and property_exists ( $this, '_BillLine' ) and property_exists ( $this, '_BillLine' )) { // Copy BillLines
      $crit = array('refType' => get_class ( $this ), 'refId' => $this->id);
      $line = new BillLine ();
...
// END - ADD BY TABARY - TOOLTIP  
  ...
  public function setAllDefaultValues($onSave = false) {
    ...
    foreach ( $defaultValues as $field => $value ) {
      if ($onSave) {
        if ((!$this->$field and !$this->id) or $this->isAttributeSetToField ( $field, 'readonly' )) {
          $def = $this->getDefaultValue ( $field, ($this->id)?true:false);
          if ($def !== null)
            $this->$field = $def;
        }
      } else {
...
// END - ADD BY TABARY - TOOLTIP    
  ...
  public function getValidationScript($colName) {
    ...
    if (pq_substr ( $colName, 0, 2 ) == 'id' and pq_strlen ( $colName ) > 2) { // SELECT => onChange
      ...
      if ($colName == 'idStatus' or $colName == $typeName or pq_substr ( $colName, - 12 ) == 'PlanningMode') {
...
//       }
      ...
      if ($statusMail->alertToContact) {
        if (property_exists ( $this, 'idContact' )) {
          $contact = new Affectable ( $this->idContact );
          if($contact->isUser){
            $newDest = "###" . $contact->id . "###";
