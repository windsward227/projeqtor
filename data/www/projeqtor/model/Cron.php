    ...
    foreach ($lstIMb as $mb){ 
      ...
      foreach ($mailsIds as $mailId){
        ...
        if (! $senderId and $securityConstraint!='1') {
        	traceLog("Email message received from '$sender', not recognized as resource or user or contact : message not stored as new ticket to avoid spamming");
            if(!$afterMailTreatment or $afterMailTreatment == 'markAsReadMail'){
          	     $inputMailbox->markMailAsRead($mailId);
      	    }else{
        	     $inputMailbox->deleteMail($mailId);
    ...
    //end gautier
	  ...
	  foreach ($mailsIds as $mailId) {
        ...
        if(!SqlElement::class_exists($class) or !$obj){
  		...
  		debugTraceLog("User corresponding to email address is #$senderId");
  		if (! $senderId) {
  			traceLog("(2) Email message received from '$sender', not recognized as resource or user or contact : message not stored as note to avoid spamming");
  		    if(!$afterMailTreatment or $afterMailTreatment == 'markAsReadMail'){
  		    	$mailbox->markMailAsRead($mailId);
  		    }else{
  		    	$mailbox->deleteMail($mailId);
