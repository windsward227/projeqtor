...
// TEST - New - Stop
  ...
  public function control(){
    ...
    if (!$this->idProfile or pq_trim($this->idProfile)=='') {
      if(!$affectable->idProfile or pq_trim($affectable->idProfile)==''){
    ...
    if (!$affectable->isResourceTeam and ! (property_exists($this,'_automaticCreation') and $this->_automaticCreation)) {
  	...
  	if(!$this->id or ($this->idle==0 and $old->idle==1)){
    	...
    	foreach ($listPlanningElement as $pel){
    	  ...
    	  if($assExist and $assExist->id){
    	   ...
    	   $pel->automaticAssignmentSave($this->idResource,$res->idRole);
    	  }
  	 }
  	}
