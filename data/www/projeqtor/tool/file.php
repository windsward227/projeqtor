...
function getThumbFileName($imageFile,$size) {
	...
	$imageFile=pq_str_replace($root, '../files/thumbs', $imageFile);
	$imageFile=pq_str_replace('\\', '/', $imageFile); 
	if (!$size) return $imageFile;
	$ext=pq_strtolower(pathinfo($imageFile, PATHINFO_EXTENSION));
	return pq_substr($imageFile,0,pq_strlen($imageFile)-(pq_strlen($ext)+1)).'_thumb'.$size.'.'.$ext;
}
