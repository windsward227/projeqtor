...
$version = "V10.0.4"; // Version of application : Major / Minor / Release
$build = "0317"; // Build number. To be increased on each release
 ...
 */
function getListForSpecificRights($specific, $includePool=false, $includeUsers=false) {
  global $user;
  if (!isset($user)) {
    $user=getSessionUser();
  }
  if ($user->allSpecificRightsForProfilesOneOnlyValue($specific, 'NO')) {
    ...
    if ($includeUsers) $table=SqlList::getList('Affectable','name',null,true);
    else $table=SqlList::getList(($includePool)?'ResourceAll':'Resource','name',null,true);
    ...
    $table=array();
    if ($includeUsers) $fullTable=SqlList::getList('Affectable','name',null,true);
    else $fullTable=SqlList::getList(($includePool)?'ResourceAll':'Resource','name',null,true);
